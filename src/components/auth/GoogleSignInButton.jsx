...
export function GoogleSignInButton({
  ...
  const handleClick = useCallback(async () => {
    if (loading || !clientId) {
      if (!clientId) toast.error('Google sign-in is not configured.');
      return;
    }
    ...
    try {
      window.google.accounts.oauth2
        .initTokenClient({
          ...
          error_callback: (err) => {
            setLoading(false);
            if (err?.type === 'popup_closed_by_user' || err?.type === 'user_logged_out') return;
  ...
  }, [clientId, scriptReady, signInWithGoogle, router, redirectTo, loading]);
