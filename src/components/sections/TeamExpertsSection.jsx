...
export default function TeamExpertsSection() {
  ...
  useEffect(() => {
    contentApi.getPage('homepage')
      .then((res) => {
        const d = res?.page?.content?.teamExperts;
      ...
      <div className="container">
        <div className="text-center mb-10 md:mb-12">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.1 }} className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold mt-2 mb-3">
            {data.title}
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.2 }} className="text-sm md:text-base text-muted-foreground max-w-xl mx-auto">
            {data.description}
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-14">
          {data.experts.map((expert, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              <div className="relative mb-5 mx-auto w-36 h-36 md:w-44 md:h-44 rounded-full overflow-hidden border-[3px] border-border group-hover:border-primary/50 transition-colors">
                <img src={getImgSrc(expert.image) || `https://ui-avatars.com/api/?name=${encodeURIComponent(expert.name)}&size=200&background=c4975a&color=fff`} alt={expert.name}
                  className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                  loading="lazy"
                  onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(expert.name)}&size=200&background=c4975a&color=fff`; }}
                />
              </div>
        ...
        <div className="grid grid-cols-3 gap-4 md:gap-6">
          {data.stats.map((stat, index) => {
            const IconComp = STAT_ICONS[stat.icon] || Users;
            return (
              <motion.div key={index} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                  <IconComp className="w-5 h-5 md:w-6 md:h-6 text-primary" />
                </div>
                <div className="text-center sm:text-left">
                  <p className="text-xl md:text-2xl font-serif font-bold text-gradient-gold">{stat.value}</p>
