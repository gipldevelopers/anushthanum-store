...
import { Shield, Award, Sparkles, Users, Truck, HeartHandshake } from 'lucide-react';
import { contentApi } from '@/services/contentApi';

const ICON_MAP = { Shield, Award, Sparkles, Users, Truck, HeartHandshake };
...
export default function WhyChooseUsSection() {
  ...
  useEffect(() => {
    contentApi.getPage('homepage')
      .then((res) => {
        const d = res?.page?.content?.whyChooseUs;
      ...
      <div className="container relative">
        ...
        <div className="text-center mb-10 md:mb-12 relative">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.1 }} className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold mt-2 mb-3 text-background">
            {data.title}
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.2 }} className="text-sm md:text-base text-background/60 max-w-xl mx-auto">
            {data.description}
          </motion.p>
        </div>
          ...
          {data.reasons.map((reason, index) => {
            ...
            return (
              <motion.div key={index} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <h3 className="text-base md:text-lg font-serif font-semibold mb-1.5 text-background">{reason.title}</h3>
