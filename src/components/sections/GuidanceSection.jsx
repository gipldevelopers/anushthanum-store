...
import { Heart, AlertTriangle, Compass, BookOpen, Shield, ArrowRight } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { contentApi } from '@/services/contentApi';

const ICON_MAP = { Heart, AlertTriangle, Compass, BookOpen, Shield };
...
export default function GuidanceSection() {
  ...
  useEffect(() => {
    contentApi.getPage('homepage')
      .then((res) => {
        const d = res?.page?.content?.guidance;
      ...
      <div className="container">
        <div className="text-center max-w-2xl mx-auto mb-10 md:mb-12">
          ...
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.1 }} className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold mb-3">
            {data.title} <span className="text-gradient-gold">{data.titleHighlight}</span>
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.2 }} className="text-sm md:text-base text-muted-foreground">
            {data.description}
          </motion.p>
        </div>
            ...
            return (
              <motion.div key={index} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <Link href={card.link || '#'}
                  ...
                  <h3 className="font-serif font-semibold text-base mb-1.5 group-hover:text-primary transition-colors">{card.title}</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-5">
            ...
            <div className="flex-1 text-center md:text-left">
              <h3 className="font-serif font-bold text-lg mb-1.5">{data.promiseTitle}</h3>
              <p className="text-sm text-muted-foreground leading-relaxed">{data.promiseText}</p>
            </div>
            <Button asChild className="flex-shrink-0">
              <Link href={data.promiseButtonLink || '/disclaimer'}>
                {data.promiseButtonText}
